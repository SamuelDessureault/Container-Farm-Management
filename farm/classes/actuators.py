...
class ACommand:
    ...
    def __init__(self, target:Type, data:dict):
        self.target_type = target
        self.data: dict = data
...
class AState:
    """Class for actuators to return their state. Defines all possible returns types via enums.
    """
    class Type(str, Enum):
        BUZZER = 'buzzer'
        FAN = 'fan'
    ...
    def __init__(self, type: Type, state: dict) -> None:
        self.sender_type = type
        self.state = state
    def __repr__(self):
        return f'The state of this {self.sender_type} is {self.state}'
    def export_json(self):
        return {"state": self.state, "sender": self.sender_type}.__str__()
